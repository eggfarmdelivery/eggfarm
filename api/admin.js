// POST /api/admin
// body: { action: 'login'|'update-order'|'photo'|'get-settings'|'drive-info'|'drive-cleanup', ...data }
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'update-order') {
      ...
      if (status === 'CANCELLED' || status === 'AUTO_CANCEL' || status === 'STOCK_CANCEL')
                                   await sheetsUpdate(`주문내역!S${r}`, [[now]]);
    ...
    if (action === 'drive-info') {
      ...
      return res.status(200).json({
        ...
        limit: parseInt(q.limit || 15 * 1024 * 1024 * 1024),
      });
    }

    // ── Drive 사진 일괄 삭제 ──
    if (action === 'drive-cleanup') {
      const { months } = data;
      if (!months) return res.status(400).json({ error: 'months 필요' });
      const cutoff = new Date();
      cutoff.setMonth(cutoff.getMonth() - parseInt(months));
      const drive = await getDrive();
      const list = await drive.files.list({
        q: `name contains 'delivery_' and createdTime < '${cutoff.toISOString()}' and trashed = false`,
        fields: 'files(id)',
        pageSize: 200,
      });
