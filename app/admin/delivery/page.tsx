...
export default async function DeliveryPage({
  ...
  if (campaignId) {
    const { data } = await admin
      .from("b2c_order")
      .select(
        "id, status, total_amount, created_at, delivery_photo_url, account(nickname, phone, address, address_dong, address_ho, entrance_password), b2c_order_item(quantity, product(name))"
      )
