...
export default async function OrdersPage() {
  ...
  const { data: rawOrders } = await supabase
    .from("b2c_order")
    .select(
      "id, order_type, status, total_amount, delivery_fee, delivery_photo_url, payment_confirmed_at, created_at, campaign_id, cancel_reason, refund_bank_name, campaign(delivery_date), b2c_order_item(id, quantity, unit_price, product_id, product(name, photo_url))"
    )
