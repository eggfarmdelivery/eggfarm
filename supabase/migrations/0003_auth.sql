...
-- 여러 번 실행해도 안전하도록 IF NOT EXISTS / IF EXISTS 가드 사용
-- =========================================================

alter table account add column if not exists auth_user_id uuid unique references auth.users(id) on delete cascade;
alter table account add column if not exists address text;
alter table account add column if not exists entrance_password text;
...
drop policy if exists "본인 계정 조회" on account;
create policy "본인 계정 조회" on account
  for select using (auth.uid() = auth_user_id);

drop policy if exists "본인 계정 생성" on account;
...
drop policy if exists "본인 계정 수정" on account;
